...
const CASES = [
  { key: "tables-add", route: "/app/tables", trigger: "Add table" },
  { key: "menu-item", route: "/app/menu/items", trigger: "Add item" },
  { key: "ingredient", route: "/app/inventory/ingredients", trigger: "Add ingredient" },
  { key: "uom", route: "/app/inventory/setup", trigger: "Add unit" },
  { key: "vendor", route: "/app/purchasing/vendors", trigger: "Add vendor" },
...
const run = async () => {
  ...
  for (const c of CASES) {
    ...
    const first = page.locator(
      '[data-slot="dialog-content"] input:not([type=hidden]):not([disabled]), [role="dialog"] input:not([type=hidden]):not([disabled])',
    ).first();
    let typed = null;
    ...
    if (rec.escape.dialogsOpen === 0) {
      ...
      if (o2.opened) {
        rec.reopenValue = await page.evaluate(() => {
          const d = document.querySelector('[data-slot="dialog-content"], [role="dialog"]');
          const i = d?.querySelector('input:not([type=hidden]):not([disabled])');
