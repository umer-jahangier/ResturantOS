 ...
 *
 * <p>The row is a CSS grid whose template changes at `sm`. At 390px each branch is a stacked block
 * with its details under its name; from 768px up the same markup lines up into columns under the
 * header. Two separate trees (one `sm:hidden`, one `hidden sm:block`) would double the row count in
      ...
      <ul className="divide-y">
        {branches.map((branch) => (
          ...
          >
            ...
            <p className="col-start-1 flex min-w-0 items-center gap-1.5 text-muted-foreground sm:col-start-2">
              <MapPin className="size-3.5 shrink-0 sm:hidden" aria-hidden="true" />
              <span className="truncate">{branch.address || "No address set"}</span>
            </p>

            <p className="col-start-1 flex items-center gap-1.5 text-muted-foreground sm:col-start-3">
              <Clock className="size-3.5 shrink-0 sm:hidden" aria-hidden="true" />
