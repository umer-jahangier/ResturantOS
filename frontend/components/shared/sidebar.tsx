 ...
 *       this entire component from the DOM on `/app/pos/**` (`app/(tenant)/layout.tsx` returns
 *       early on `isOperatorRoute`), so no rail, glowing or otherwise, is reachable from a
 *       terminal. It is also static: no animation, no transform, no filter, no containing block
 *       — so it is invisible to `zone-containment.test.ts`'s six creators and cannot print itself
 *       onto a receipt.</li>
 * </ol>
 *
