...
PGPASSWORD="$POSTGRES_SUPERUSER_PASSWORD" psql -h 127.0.0.1 -U "$POSTGRES_SUPERUSER" -d postgres \
  -f init/03-grant-schema-privileges.sql -v ON_ERROR_STOP=1 -q

echo "==> Ensuring auth refresh lookup function owner (RLS bypass)..."
PGPASSWORD="$POSTGRES_SUPERUSER_PASSWORD" psql -h 127.0.0.1 -U "$POSTGRES_SUPERUSER" -d auth_db \
  -f init/04-auth-refresh-lookup-owner.sql -v ON_ERROR_STOP=1 -q
