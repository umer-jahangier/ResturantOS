...
data:
  ...
  # Second-factor step-up. TRUE here on purpose so any environment that does not
  # explicitly opt out keeps it. The dev overlay sets it false, temporarily, because
  # there is no enrolment UI yet: the seeder writes TOTP secrets to a local state
  # directory, so a human with only the password cannot get past the owner, tenant
  # admin or accountant. That is a missing FEATURE, not a reason to weaken prod —
  # prod keeps this on.
    ...
    spec:
      containers:
        - name: clickhouse
          ...
          resources:
            # 2Gi, not 1Gi. The 1Gi cut was measured against an EMPTY ClickHouse
            # and held there; a seeded one OOMKilled 19 times (exit 137) because
            # max_server_memory_usage bounds QUERY memory, not the whole process —
            # part loading, marks and the page cache at startup sit outside it.
            # memory.xml still keeps queries well inside this ceiling.
...
<clickhouse>
    <!-- MUST track the container limit in 14-clickhouse.yaml. When that limit was raised
         1Gi -> 2Gi to stop an OOMKill, this was left at 768MiB and the two then contradicted
         each other: the container permitted 2Gi, ClickHouse capped ITSELF at 768MiB, its RSS
         was already 1.06GiB from caches, and it then refused its own queries with
         "memory limit exceeded ... maximum: 768.00 MiB". reporting-service's startup check is
         an aggregation, so it was refused, crash-looped, and failed every single deploy.
         1.5GiB inside a 2Gi container leaves headroom for the OOM killer never to be the one
         that decides. Change both numbers together or neither. -->
