...
**D-30-01 — Terminal management is a tenant UI surface.** Register a device, name it, assign it to
a branch, see its last-seen time, enable/disable it, rotate its token, delete it. No SQL.

**D-30-02 — Device health must be visible, and silence must be loud.**
...
**D-30-03 — Employee ↔ device-user mapping is explicit and manageable.**
...
**D-30-04 — Testable with no hardware; the terminal is sign-off only.**
...
**D-30-05 — Duplicate and out-of-order punches are expected, not exceptional.**
...
**D-30-06 — Do not weaken the device auth path.** `/iclock` is public by necessity (a terminal
