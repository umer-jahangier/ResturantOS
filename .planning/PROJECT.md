...
- [x] Inventory & procurement master-data + UI: ingredient categories as a 3-level tree (CRUD/re-parent/archive-in-use-refused), ingredient master data (UOM conversions, allergens, par/reorder) manageable in-UI, recipe view/revise with a live plate-cost panel, three-state recipe coverage (fixes the future-dated-recipe origin bug), vendor item catalog with append-only effective-dated pricing, catalog-driven PO-line picker replacing the hand-typed ingredient UUID, real spend-by-category analytics (mock resolver deleted), and stock receipts/transfers/counts/opening-balances driveable from the UI over a real on-hand read endpoint — *Validated in Phase 08.2 (2026-07-25): 20 plans, 8/8 success criteria, additive V5/V6 migrations (depletion loop intact); 3 code-review criticals (transfer cross-branch authz, PO zero-price & negative-qty approval-tier weakening) fixed & IT-covered post-verification; INV-13..15, PUR-07..08, INV-01 UI re-open*
...
---
*Last updated: 2026-07-25 — Phase 08.2 (Inventory Master Data & Procurement Catalog) complete: ingredient categories + master data, recipe view/revise with live plate cost, three-state coverage (origin bug fixed), vendor item catalog with append-only pricing, catalog-driven PO-line picker, real spend-by-category analytics, and full stock-operations UI over a real on-hand endpoint — 20 plans, 8/8 criteria, additive migrations (depletion loop intact); 3 code-review criticals fixed and IT-covered post-verification.*
