...
- [x] **Phase 34: Visual Design Language** - glass, depth and motion on top of phase 20's tokens. **COMPLETE (2026-08-12): 8 of 8 plans.** Landed: zoning spine + containment and print-safety gates (34-01), composite-aware contrast + solid-first glass/depth tokens (34-02), motion vocabulary + two-direction reduced-motion gate (34-03), surface primitives + dependency budget (34-04), data-state character with a measured salience margin and a runtime distinguishability gate (34-05), dashboard portlets + a mask-based chart reveal with byte-identical geometry + a mount-keyed count-up (34-06), login/console/settings + the brand-hue sweep (worst case 5.34:1 at 125 deg) (34-07), SPEC + POS latency (79-99ms) + bundle delta (34-08). **Eight vacuous gates found and catalogued in SURFACE-MOTION-SPEC.md section 7** - including all three KDS assertions, which had been running against the station picker rather than the board, and which uncovered a real entrance animation on every ticket fragment. **Not done: the platform console is restyled but was not re-verified in a browser this session, and the onboarding surface named in 34-CONTEXT does not exist.**
