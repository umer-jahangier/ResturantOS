...
Plan: 17 of 18 (gap-closure wave 1 of 5; other wave-1/2/3/4/5 plans in flight in parallel — 10-08..10-10, 10-12..10-14, 10-18)
Status: 10-16 complete — purchasing-service now refuses to start when restaurantos.encryption.key is unset OR blank (EncryptionRequiredConfig, a BeanFactoryPostProcessor checking both the raw property and EncryptionService bean presence); VendorService.apply's silent null-out-on-missing-key branch is removed (EncryptionService is now a required constructor dependency). Proven via VendorEncryptionFailFastIT: real ApplicationContextRunner startup-failure test + negative control + full-context raw-JDBC plaintext check. Manual negative control (restore old null-out branch) confirmed the test fails as expected before reverting. Full purchasing-service `mvn verify`: 38/38 green. 10-15 also complete in parallel (analytics page usable across periods/vendors). 10-17 also complete in parallel (FIN-05 AR scope decision recorded: AR IN scope via corporate/house accounts, split Phase 10 (10-18) / Phase 7 (07-05); FIN-05 flipped Complete->In Progress; phase plan count 17->18). Remaining: 10-08..10-10, 10-12..10-14, 10-18.
Last activity: 2026-07-13 — Completed 10-16 (VendorService encryption fail-fast — 2 tasks, 2 commits, a3a5ad8/c99323b)
