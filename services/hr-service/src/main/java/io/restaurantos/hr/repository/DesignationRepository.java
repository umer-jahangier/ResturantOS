...
@Repository
public interface DesignationRepository extends JpaRepository<DesignationEntity, UUID> {

    List<DesignationEntity> findAllByTenantIdOrderByNameAsc(UUID tenantId);

    Optional<DesignationEntity> findByIdAndTenantId(UUID id, UUID tenantId);

    /**
     * Case- and whitespace-insensitive name lookup, matching the FUNCTIONAL unique index
     * {@code ux_designations_tenant_name} on {@code (tenant_id, lower(trim(name)))}.
     *
    ...
    @Query("""
           ...
           """)
    Optional<DesignationEntity> findByNormalisedName(@Param("tenantId") UUID tenantId,
