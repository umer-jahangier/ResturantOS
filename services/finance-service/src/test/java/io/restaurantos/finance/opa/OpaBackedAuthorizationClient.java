 ...
 * test that mocks the thing that broke proves nothing). Per decision 10-05-A, finance-service
 * has its OWN copy of the {@code AuthorizationClient} Feign interface (not shared with
 * purchasing-service), so this class implements THAT copy — it is intentionally not shared with
 * {@code io.restaurantos.purchasing.opa.OpaBackedAuthorizationClient}.
 *
 * <p>Reproduces exactly what {@code AuthorizeService.authorize()} / {@code DefaultOpaClient} do
 * in production: builds a snake_case {@link OpaInput} from the caller's identity (held in a
 * mutable {@link TestPrincipal}) and the service's own {@code AuthorizePayload} (action string
 * passed through with ZERO rewriting), then POSTs it straight to a real OPA server evaluating
 * the real {@code policies/} bundle at {@code /v1/data/restaurantos/{module}/allow}. No mocking
 * anywhere in this path — the rego decides allow/deny for real.
 */
