...
public class ReportingSecurityConfig {
    ...
    private final ReportingInternalServiceFilter internalServiceFilter;

    public ReportingSecurityConfig(ReportingInternalServiceFilter internalServiceFilter) {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http,
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Dashboard WebSocket handshake path — permit at HTTP level; the WS handler
                // does its own JWT-in-query-param check (mirrors kitchen-service's KDS path).
                .requestMatchers("/api/v1/reporting/dashboard/**").permitAll()
                .anyRequest().authenticated()
            )
            .addFilterBefore(internalServiceFilter, UsernamePasswordAuthenticationFilter.class)
            .addFilterAfter(jwtAuthenticationFilter, ReportingInternalServiceFilter.class);
        return http.build();
    }
}
